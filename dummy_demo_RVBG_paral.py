...
#def get_results(last_sol, notimproved, j):
def get_offspring(j):
    #for j in range(0,10): 
    ...
    for i in range(ini_g+1, gens):
        ...
        if notimproved >= 15:
            ...
            pop, fit_pop = mu_lambda(pop, fit_pop, offspring)
...
#get_results(last_sol, notimproved, j)

for j in range(0,10): #10 runs 
    get_offspring(j)
    ###result = pool.map(partial(get_offspring,pop=pop,fit_pop=fit_pop), [i for i in range(ini_g+1, gens)])
#pool.map(get_offspring, [j for j in range(0,10)])
#pool.close()
    #pool.join()a
