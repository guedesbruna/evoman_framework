...
def crossover(pop):
    ...
    for p in range(0, pop.shape[0], 2):
        ...
        for f in range(0, n_offspring):
            ...
            tau = 1 / (2*npop ** (1 / 2))** (1 / 2)
            tau_prima = 1 / (2*npop )** (1 / 2)
            sigma = 1
            sigma = sigma * e ** (tau_prima * np.random.normal(0, 1)+ tau * np.random.normal(0, 1))  # mutation step size
...
def mu_lambda(pop, fit_pop, offspring):
    ...
    # pick the best children and replace until the population size is reached
    i = 1
    new_pop = pop
...
for j in range(0, 10):  # add to the end j+=1 e tb resetear logs
    ...
    for i in range(ini_g + 1, gens):
        ...
        fit_offspring = evaluate(offspring)[:,0]  # evaluation
        fit_offspring_ind = evaluate(offspring)[:,1]
        pop = np.vstack((pop, offspring))
        fit_pop = np.append(fit_pop, fit_offspring)
        fit_pop_ind = np.append(fit_pop_ind, fit_offspring_ind)

        best = np.argmax(fit_pop)
        best_ind= np.argmax(fit_pop_ind) # best solution in generation
        fit_pop[best] = float(evaluate(np.array([pop[best]]))[:,0][0]) # repeats best eval, for stability issues
        fit_pop_ind[best_ind] = float(evaluate(np.array([pop[best_ind]]))[:,1][0])
        ...
        fit_pop_ind = fit_pop_ind[chosen]
        ...
        if notimproved >= 15:
            ...
            pop, fit_pop, fit_pop_ind = mu_lambda(pop, fit_pop, offspring)
